...
for class=1:K
    ...
    for a_pixel_that_is_maybe_clouded = 1:sum(sum(mask_maybe_cloud)) %N_maybe
        I_i = X(a_pixel_that_is_maybe_clouded,:); 
  ...
  %      I_maybe_cloud_corrected =  I_i .* sum(exp(thicknesses'*g'),1)/M;
        ...
        Xcorrected(indices_row_maybe_cloud(a_pixel_that_is_maybe_clouded), indices_column_maybe_cloud(a_pixel_that_is_maybe_clouded),:) = I_maybe_cloud_corrected; 
        corrected_points(indices_row_maybe_cloud(a_pixel_that_is_maybe_clouded), indices_column_maybe_cloud(a_pixel_that_is_maybe_clouded)) = 1;
        shadowmap(indices_row_maybe_cloud(a_pixel_that_is_maybe_clouded), indices_column_maybe_cloud(a_pixel_that_is_maybe_clouded)) = max(thicknesses);
